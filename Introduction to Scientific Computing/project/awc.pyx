...
#d is the distance matrix here
@cython.boundscheck(False)
def get_lambda_hash(h, np.ndarray[DTYPE_t, ndim=2] d, np.ndarray[DTYPE_t, ndim=1] L_T):
    """Extract the values computed by get_lambda_table()"""
    
    cdef double h_ratio = 1.95
...
def get_lambda_table(n, m):
    ...
    cdef double h_ratio = 1.95
    cdef np.ndarray x = np.linspace(0 + 1./m, h_ratio + 1./m, m+1, dtype=DTYPE)
    cdef np.ndarray a = spec.betainc((n+1.) / 2, 0.5, 1. - (x/2.)**2)
    return a / (2-a)

@cython.boundscheck(False)
def init_weights(np.ndarray[DTYPE_t, ndim=2] dist_matrix, np.ndarray[DTYPE_t, ndim=1] H, n_0, discrete=False):
    """Initialize the matrix of weights_"""
    
    cdef np.ndarray dmatrix = np.sort(dist_matrix, axis=1)
    cdef np.ndarray v = dmatrix[:, n_0]
    cdef int n = np.size(dist_matrix,0)
    cdef int i
    cdef int j
    if discrete == True:
        for i in range(n):
    ...
    cdef np.ndarray weights = np.zeros((n,n), dtype=DTYPE)
    for i in range(n):
        h_closest = H[-1]
        for h in H:
            if h >= v[i]:
...
def KL_init():
    ...
    cdef int m = 2000
    cdef np.ndarray e1 = np.linspace(0, 1, m + 1, dtype=DTYPE)
    cdef np.ndarray e = np.repeat([e1], m + 1, axis=0)
    cdef np.ndarray q = e.T
...
@cython.boundscheck(False)
def get_h_intervals(np.ndarray[DTYPE_t, ndim=2] dmatrix, n_0, discrete=False, speed=1.5):
    """Compute the sequence of radii h_k for all steps"""
      
    cdef double h_ratio = 1.95
    cdef np.ndarray dist_matrix = np.sort(dmatrix)
    cdef int n = np.size(dist_matrix, 0)
    ### Idea
    cdef double aa = 2 ** 0.5

    cdef h_final = [np.min(dist_matrix[:, n_0-1])]
    #for discrete
    if discrete == True:
    ...
    cdef double h_max = np.max(dist_matrix)
    ...
    for i in range(4):
        a.append(a[-1]* 1.5)
    return np.array(a), dist_matrix



@cython.boundscheck(False)
def init(np.ndarray[DTYPE_t, ndim=2] X, np.ndarray[DTYPE_t, ndim=2] dist_matrix, n_neigh=-1, dim=-1, discrete=False, speed=1.5):
    ...
    if n_neigh == -1:
        ...
        #else:
        #    n_0 = 2 *s dim + 2
    cdef np.ndarray L_T = get_lambda_table(dim, 10000)
    cdef np.ndarray H, dist_ordered
    H, dist_ordered = get_h_intervals(dist_matrix, n_neigh, discrete, speed)
    cdef np.ndarray v = dist_ordered[:, n_neigh-1].clip(min=H[0])
    cdef np.ndarray weights = init_weights(dist_matrix, H, n_neigh, discrete) 
    cdef np.ndarray T = np.zeros((n, n))
    KL = KL_init()
    return L_T, dist_matrix, H, v, weights, T, KL


@cython.boundscheck(False)
def cluster_step(l,np.ndarray[DTYPE_t, ndim=2] weights, np.ndarray[DTYPE_t, ndim=1] v, n, k, np.ndarray[DTYPE_t, ndim=1] L_T, np.ndarray[DTYPE_t, ndim=2] T, KL,np.ndarray[DTYPE_t, ndim=2] dist_matrix, np.ndarray[DTYPE_t, ndim=1] H):
    """ Make one iteration of the AWC algorithm"""

    #for each point, get the number of points that are closer than H[k-1] and
    #  are considered in the same cluster
    cdef np.ndarray D2 = dist_matrix <= H[k-1]
    np.fill_diagonal(D2, False)
    cdef np.ndarray P =  D2 * weights

    cdef np.ndarray neighbour_numbers = np.sum(P, axis = 1)


    cdef np.ndarray max_dist = np.max(dist_matrix, axis=1)
    
    cdef np.ndarray t_1 = (neighbour_numbers - P).T
    cdef np.ndarray t_12 = np.inner(P, P)
    cdef np.ndarray t_12x = np.inner(P, D2)
    cdef np.ndarray gg1 = (t_1 == t_12x) * (t_12 < 0.5 * t_12x)
    #gg2 = (t_1.T == t_12x.T) * (t_12 < 0.5 * t_12x.T)
    
    t_1 = t_1 - t_12x + t_12
    cdef np.ndarray q = get_lambda_hash(H[k-1], dist_matrix, L_T)
    #E = (max_dist[i] < H[k-1]) * ( max_dist[i+1:] < H[k-1])
    #q[E] = 1. / get_lambda_hash(np.maximum(max_dist[i], max_dist[i+1:][E]), dist_matrix[i, i+1:][E], L_T)
    
    cdef np.ndarray E = max_dist < H[k-1]
    cdef np.ndarray F = np.repeat([max_dist], n, axis = 0)
    cdef np.ndarray R = np.maximum(F.T, F)
    q[E, :][:, E] = get_lambda_hash(R[E, :][:, E], dist_matrix[E, :][:, E], L_T)
    
    cdef np.ndarray  t = t_1 + t_1.T - t_12
    ...
    cdef np.ndarray I = (dist_matrix <= H[k]) * (dist_matrix > 0) * (T != float("inf")) * (np.isnan(T) == False)
    weights[I] = 1 * (T[I] <= l)
...
@cython.boundscheck(False)
def connect_outliers(np.ndarray[DTYPE_t, ndim=2] weights, n_outliers, np.ndarray[DTYPE_t, ndim=2] dist_matrix, n):
    """ Connect outliers to their nearest clusters_"""
    
    cdef np.ndarray N = np.sum(weights, axis=1)
    cdef np.ndarray outliers = N <= n_outliers
    cdef np.ndarray distance_order = np.argsort(dist_matrix, axis=1)
    cdef int i, j
    cdef int I = n
    for i in range(I):
        if outliers[i] == True:
    ...
    cdef np.ndarray outliers = np.where(N <= n_outliers)[0]
    cdef np.ndarray distance_order = np.argsort(dist_matrix, axis=1)
    cdef J = len(outliers)
    cdef np.ndarray w
    for i in range(outliers):
            for j in range(1, I):
                if j not in outliers[distance_order[outliers[i],j]]:
                    w = weights[j]
                    weights[i] = w
                    weights.T[i] = w
                    break
...
@cython.boundscheck(False)
def clustering_from_weights(np.ndarray[DTYPE_t, ndim=2] weights):
    ...
    clusters = []
    cdef np.ndarray adjmatrix = np.zeros((n, n), dtype=np.int8)
    cdef i, j
    for i in range(n):
        for j in range(n):
            if weights[i, j] >= 0.5:
...
class AWC():
    ...
    def awc(self, l, np.ndarray[DTYPE_t, ndim=2] X=None,np.ndarray[DTYPE_t, ndim=2] dmatrix=None):
        ...
        cdef int n = np.size(self.dmatrix, 0)
        cdef int k
        cdef int K = len(H)
        for k in range(1, K):
